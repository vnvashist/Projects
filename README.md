# Projects
[Red Wine Price Prediction](#red-wine-price-prediction)
\
...
```
\
I used three metrics to determine how well my model was performing. MAE, MSE, and R2. All three of these helped me hone in on what I needed to change in my methods to iteratively improve my model. Of course, I already knew that this first model was not going to perform that well. The MAE and MSE scores were far too high and the R2 score was negative... Not great
\
Accordingly, I decided to do two things. One was revisit the feature engineering section. Here, instead of just using static frequency encoding, I decided to multiply the frequency encoded columns with each other to help cleary state the relationships they may have with each other. For example, Country_Freq * Wine_Age, or Country_Freq * Rating. This helps the model catch on the relationships explicity, rather than trying to find them out on its own. Secondly, I decided to implement a grid search for my hyperparameters. Just using static values was missing out on the potential for other, better hyperparameters:
```
param_grid = {
